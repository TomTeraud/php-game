...
class GamePauseHandler implements MessageHandlerInterface
{
    /**
    ...
    public function handle(ConnectionInterface $from, array $data, GameServer $server): void
    {
        ...
        $server->getGameEngine()->pauseServerGameLoop();
        ...
        foreach ($server->getClients() as $client) {
            $client->send(json_encode(['type' => 'game_status', 'message' => 'Game paused.']));
        }
    }
}
